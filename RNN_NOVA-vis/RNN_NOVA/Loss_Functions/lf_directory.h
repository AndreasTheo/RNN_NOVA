...
double LF(double* yV, const int aSize, double* targetV, std::string* lF_Type, std::string* prevAF_Type, bool regularize) {
	const std::unordered_map<std::string, int> string_to_case_LF{
		{ "mse",1 },
		{ "crossentropy",2 },
		{ "exponential",3 }
	};
...
void LFtoYDeriv(double* gradV, double* yV, const int aSize, double* targetV, std::string* lF_Type, std::string* prevAF_Type, bool regularize) {
	...
	double* yDeriv; //Activation Function Derivative
	...
	case 1:
	{
		...
		{
			MSE_DerivTo_Sigmoid_YGRAD(gradV, yV, aSize, targetV);
		}
		else if (*prevAF_Type == "tanh")
		{
			MSE_DerivTo_Tanh_YGRAD(gradV, yV, aSize, targetV);
		}
	...
	case 2:
	{
		...
		{
			CrossEntropy_Binary_DerivTo_Sigmoid_YGRAD(gradV, yV, aSize, targetV);
		}
		else if (*prevAF_Type == "tanh")
		{
